...
<section>
	<article>';
		if(isset($_SESSION['session']) and $_SESSION['session'] == true){
			$requete = $bdd->prepare("SELECT * FROM forum_souscategories FSO JOIN forum_categories FC ON FSO.id_categorie = FC.id_categorie WHERE FSO.id_souscategorie = :id_souscat");
			$requete->execute(array('id_souscat'=>intval($_GET['id_souscat']))) or die(print_r($requete->errorInfo()));	
			$donnees = $requete->fetch();
			echo'<p class="ariane">> <a href="index.php">Forum</a> > '.htmlspecialchars($donnees['titre_cat']).' > <a href="voir_sujet.php?id_souscat='.intval($_GET['id_souscat']).'">'.htmlspecialchars($donnees['titre_souscat']).'</a> > Ajout d\'un sujet</p><br>';
			...
			************* dans la souscat **************/
			...
			if(isset($_GET['ajout'])){
				...
				**************** dans le sujet *************/
				...
				$requete = $bdd->prepare("INSERT INTO forum_sujets (titre_sujet,sousTitre_sujet,id_souscat,annonce_sujet,date_creation_sujet, date_modification_sujet,date_derniere_reponse,id_membre,message_sujet) VALUES(:titre,:soustitre,:id_souscat,:annonce,:date_c,:date_m,:date_d,:id_m,:message)");
				$requete->execute(array('titre'=>$_POST['titre'], 'soustitre'=>$_POST['soustitre'], 'id_souscat'=>intval($_GET['id_souscat']),'annonce'=>$annonce, 'date_c'=>time()+3600, 'date_m'=>null,'date_d'=>time()+3600, 'id_m'=>$_SESSION['id'], 'message'=>$_POST['message'])) or die(print_r($requete->errorInfo()));	
				header("Location: voir_sujet.php?id_souscat=".intval($_GET['id_souscat']));
			}
			
			echo '<h1 class="titre">'.htmlspecialchars($donnees['titre_souscat']).'</h1>'; // titre
			
			echo'
			<form id="ajout_sujet" method="post" action="ajout_sujet.php?id_souscat='.intval($_GET['id_souscat']).'&amp;ajout=ok" ">
				<fieldset>
